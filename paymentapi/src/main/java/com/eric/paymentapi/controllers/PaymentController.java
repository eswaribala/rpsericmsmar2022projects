...
public class PaymentController {
    @Autowired
	private PaymentPublishService paymentPublishSrevice;
    
    @GetMapping({"/v1.0"})
    public Order getOrderDetails() {
    	return this.paymentPublishSrevice.getOrderInfo();
    }
    
    @PostMapping({"/v1.0"})
    public ResponseEntity<String> makePayment(@RequestBody Payment payment){
    	Payment paymentObj=this.paymentPublishSrevice.savePayment(payment);
